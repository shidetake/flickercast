...
/**
 * 子供の年齢から親の年齢を計算
 * @param childAge 子供の年齢
 * @param childBirthYear 子供の誕生年
 * @param currentYear 現在の年
 * @param currentAge 親の現在年齢
 * @returns 親の年齢
 */
export function calculateParentAgeFromChildAge(
  childAge: number,
...
export function generateEducationExpenses(
  ...
  const universityChildAge = 18;
  const parentAge = calculateParentAgeFromChildAge(universityChildAge, child.birthYear, currentYear, currentAge);
...
export function expandMultiYearExpense(
  ...
  for (let i = 0; i < multiYearExpense.years; i++) {
    ...
    const parentAge = calculateParentAgeFromChildAge(childAge, child.birthYear, currentYear, currentAge);
