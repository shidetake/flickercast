...
/**
 * 子供の教育費が手動で編集されているか検出
 * @param child 子供情報
 * @param currentYear 現在の年（例: 2025）
 * @param currentAge ユーザーの現在年齢
 * @returns 手動編集されている場合true
 */
export function hasManualEdits(
  ...
  // 期待される費用を生成
  const expectedExpenses = generateEducationExpenses(child, currentYear, currentAge);

  // 自動生成費用のみを抽出
  const autoGeneratedExpenses = child.expenses.filter(e => e.autoGenerated);

  // 数が異なれば編集済み（削除された）
  if (autoGeneratedExpenses.length !== expectedExpenses.length) {
    return true;
  }

  // 各費用を比較（ID以外の全フィールド）
  for (const expected of expectedExpenses) {
    const actual = autoGeneratedExpenses.find(e => e.id === expected.id);
    if (!actual) return true;

    // 名前、金額、年齢のいずれかが異なれば編集済み
    if (
      actual.name !== expected.name ||
      actual.amount !== expected.amount ||
      actual.targetAge !== expected.targetAge
    ) {
      return true;
    }
  }
...
export function expandMultiYearExpense(
  ...
  // 子供の順番ラベル（第一子、第二子など）
  const childLabel = ['第一子', '第二子', '第三子', '第四子', '第五子'][childIndex] || `第${childIndex + 1}子`;
  ...
  for (let i = 0; i < multiYearExpense.years; i++) {
    ...
    expenses.push({
      ...
      name: `${multiYearExpense.name}（${childLabel}）`,
...
export function expandAllMultiYearExpenses(
  ...
  return child.multiYearExpenses.flatMap(expense =>
    expandMultiYearExpense(expense, child, currentYear, currentAge, childIndex)
  );
}
...
export function expandAllChildrenMultiYearExpenses(
  ...
  return children.flatMap((child, index) =>
    expandAllMultiYearExpenses(child, currentYear, currentAge, index)
  );
}
