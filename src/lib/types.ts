...
export interface Child {
  id: string;
  birthYear: number; // 誕生年（例: 2020）
  kindergartenPrivate: boolean; // 幼稚園が私立か
  elementaryPrivate: boolean; // 小学校が私立か
  juniorHighPrivate: boolean; // 中学が私立か
...
export interface SpecialExpense {
  ...
  targetAge?: number; // 支出予定年齢（子供に紐づかない支出の場合）
  childAge?: number; // 子供の年齢（子供に紐づく支出の場合）
  autoGenerated?: boolean; // 自動生成された支出かどうか
  childId?: string; // 子供に紐づく支出の場合、その子供のID
}
